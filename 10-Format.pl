...
my $outDir   = "10-Format";
my $tempDir  = "yp-temp-for-10-Format";
&myMakeDir($outDir);
&myMakeDir($tempDir);
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {   
        ...
        for (my $j=1;  $j<=$#lines1;  $j++) { 
            ...
            $name =~ s/\"//g  or  die;
            $name !~ m/\"/    or  die;
            open(outFILE1,   ">>", "$tempDir/$inputFiles_g[$i]/$name")   or die "$!";    
...
for ( my $i=0; $i<=$#tempFiles_g; $i++ ) {   
        next unless $tempFiles_g[$i] =~ m/^chr/;  
        next unless $tempFiles_g[$i] =~ m/fa$/;  
        ...
        my @lines1 = <inputFILE1>;
        open(FILE2,   ">",  "$outDir/$tempFiles_g[$i]")   or die "$!";  
        print     FILE2      "sgRNA\tPAM\tOfftargetScore\t$lines1[0]";

        for (my $j=0;  $j<=$#tempFiles1;  $j++) {   ## one cycle is one folder (one enhancer).
            next unless $tempFiles1[$j] =~ m/^chr/;  
            #next unless $tempFiles1[$j] =~ m/__$/;  
            ...
            my $bool  = 0;
            my $index = "";
            for (my $k=0;  $k<=$#tempLines1;  $k++) {  ## one cycle is one sgRNA.
                 say   $tempLines1[$k];
                 $tempLines1[$k] =~ m/^(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+/  or  die; 
                 my $name1    = $1;
                 my $gRNAPAM1 = $2;
                 my $score1   = $4;       
                 my $chrom1   = $11;       
                 ...
                 $name1 =~ m/^\"(chr[\dXY]+):(\d+)\-(\d+)_/  or  die; 
                 my $chrom2 = $1;    
                 my $start2 = $2;    
                 my $end2   = $3; 
                 $gRNAPAM1 !~ m/TTTTT/  or  die;    
                 $gRNAPAM1 =~ m/^\"(G[AGCTN]{19})([AGCTN]GG)\"$/  or  die;    
                 my $sgRNA = $1;
                 my $PAM = $2;
                 $chrom1 =~ s/\"//g    or  die;
                 $chrom1 =~ m/^chr/    or  die;
