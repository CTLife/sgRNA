...
my $inputDir = "4-find-sgRNAs";
my $outDir   = "5-select-sgRNAs";
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {   
        ...
        for (my $i=1; $i<=$#lines1; $i++) {
             ...
             my $gRNAefficacy = $6;
             (length($gRNAplusPAM) == 23)    or  die;
             $gRNAplusPAM =~ m/[AGCT]GG$/  or die;
             if( ($gRNAefficacy =~ m/^[\.\d]+$/) and ($gRNAefficacy > 0.1) and ($gRNAplusPAM !~ m/N/) and ($gRNAplusPAM !~ m/TTTTT/)  and ($gRNAplusPAM =~ m/^G/) and ($sgRNAwithEnzyme !~ m/\t$gRNAplusPAM\t/) ) { 
