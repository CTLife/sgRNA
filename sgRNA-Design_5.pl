...
open(outFILE_g2,   ">", "$outDir/removed-targetRegions.txt")   or die "$!";
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {   
        ...
        if($#lines1 < 1) {print  outFILE_g2  "$inputFiles_g[$i]\n";}
       ...
       for (my $i=0;  $i<$#lines1;  $i++) { 
           $sorted_array2d[$i][0] =~ m/^[AGCT]{21}[AGCT][AGCT]G[AG][AG]T$/  or   $sorted_array2d[$i][0] =~ m/^G[AGCT]{19}[AGCT]GG$/    or  die;
