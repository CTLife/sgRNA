...
for ( my $i=0; $i<=$#tempFiles_g; $i++ ) {   ## one cycle is one folder (one enhancer).
        ...
        for (my $j=0;  $j<=$#tempFiles1;  $j++) {   ## one cycle is one sgRNA.
            ...
            for (my $k=0;  $k<=$#tempLines1;  $k++) {  ## one cycle is one of the same sgRNAs.
                 ...
                 $name1 =~ m/\_(chr[\dXY]+):(\d+)\-(\d+)\_gR[a-z\d]+\_([AGCT]+)\_/  or  die  "\n##$name1##\n\n"; 
                 my $chrom2 = $1;    
                 my $start2 = $2;    
                 my $end2   = $3;
                 my $gRNAPAM1 = $4; 
                 $gRNAPAM1 !~ m/TTTTT/  or  die;    
                 $gRNAPAM1 =~ m/^([AGCT]{21})([AGCT][AGCT]G[AG][AG]T)$/   or   $gRNAPAM1 =~ m/^(G[AGCT]{19})([AGCT]GG)$/    or  die  "\n##$name1##\n\n\n##$gRNAPAM1##\n\n";    
                 my $sgRNA = $1;
                 my $PAM = $2;
                 $chrom1 =~ m/^chr/    or  die;
