...
offTargetAnalysis(      inputFilePath=paste(inputDir, inputFasta, sep="/"),   format="fasta",   header=FALSE,   gRNAoutputName=inputFasta,   findgRNAs=TRUE,
			...
			enable.multicore=TRUE,   n.cores.max=6,    min.gap=0,   max.gap=20,    gRNA.name.prefix=inputFasta, 
			PAM.size=6,    gRNA.size=21,  PAM="NNGRRT",   BSgenomeName=BSgenome.Mmusculus.UCSC.mm9,    chromToSearch="",
			...
			max.mismatch=3,   PAM.pattern="NNG[A|G][A|G]T$",      allowed.mismatch.PAM=4,   gRNA.pattern="",  
			...
			subPAM.activity = hash( AA = 0,
						...
						TG = 0.038961039,
						TT = 0),
			subPAM.position=c(22, 23),    mismatch.activity.file=system.file("extdata", "NatureBiot2016SuppTable19DoenchRoot.csv", package="CRISPRseek")
)
